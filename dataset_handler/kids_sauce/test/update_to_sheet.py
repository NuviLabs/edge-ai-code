...
model_path = "/home/kaeun.kim/yolov11/runs/segment/train4/weights/best_saved_model/best_float16.tflite"
from tqdm import tqdm
for row_idx, row in enumerate(tqdm(df)):
    ...
    after_path = f'/mnt/nas/data/kaeun/q4/kids/detector_qa/{dep}/{date}/{after_name}'
    ...
    if not os.path.exists(after_path):
        ...
        continue
    after_result = run_entire_pipeline(model_path, after_path, num_cls=9, food_index=[0,5], tray_index=6)
    ...
    before_path = f'/mnt/nas/data/kaeun/q4/kids/detector_qa/{dep}/{date}/{before_name}'
    ...
    if not os.path.exists(before_path):
        ...
        continue
    before_result = run_entire_pipeline(model_path, before_path, num_cls=9, food_index=[0, 5], tray_index=6)
    ...
    cell7 = worksheet.cell(row_idx+1, 16)
    cell7.value = intake 
