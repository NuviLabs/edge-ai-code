...
def nms(results: List[AIResult]) -> List[AIResult]:
    ...
    while pq:
        ...
        while pq:
            ...
            if not(box_iou(max_result.rect, detection.rect) >= 0.3 and detection.detection_class == max_result.detection_class):  # IoU threshold
...
def get_tray(detections):
    ...
    """
    tray_detections = [det for det in detections if det.detection_class == 10]
