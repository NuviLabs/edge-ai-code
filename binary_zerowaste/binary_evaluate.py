...
model.load_state_dict(torch.load("/home/kaeun.kim/kaeun-dev/nuvilab/models/best_model_yolo_m_tflite3.pth", map_location=torch.device('cpu')))
...
images = glob("/mnt/nas/data/kaeun/q4/binary_zerowaste/charlie_testset_tflite/0/*.png")  # Replace with the path to your image
gt = [0] * len(images)
pred = []
pred_imgs = []
for image in images:
    pred_imgs.append(image)
    image = Image.open(image).convert("RGB")  # Ensure the image is in RGB format
