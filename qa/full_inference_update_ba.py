...
if __name__ == "__main__":
    ...
    project_name = 'kids_detector_ba_241231_v2'
    if not os.path.exists(f'/home/kaeun.kim/kaeun-dev/nuvilab/qa/{project_name}'):
        os.makedirs(f'/home/kaeun.kim/kaeun-dev/nuvilab/qa/{project_name}')
    with open(f'/home/kaeun.kim/kaeun-dev/nuvilab/qa/{project_name}/kids_qa_ba_241231_odd_tray.csv', mode='w+', newline='') as full_file:
        full_writer = csv.writer(full_file)
        full_writer.writerow(['s3_uri_after', 'tray_area_after', 'food_area_after', 's3_uri_before', 'tray_area_before', 'food_area_before'])
        ...
        for k, v in matched_pairs.items():
            if len(v) < 2:
                ...
                pred, zw, label_exists, tray_area_after, food_area_after = run_entire_pipeline(model_path, after_name)
                pred, zw, label_exists, tray_area_before, food_area_before = run_entire_pipeline(model_path, before_name)
                if pred == -1:
                    print("no tray: ", webp)
                    continue
                s3_uri_before = f"s3://nuvi-data/{dep}/{date}/L/B/{before_name.split('/')[-1]}"
                s3_uri_after = f"s3://nuvi-data/{dep}/{date}/L/A/{after_name.split('/')[-1]}"
