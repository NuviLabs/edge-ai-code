...
#     input_data = np.expand_dims(padded_img_data.astype(np.float32) / 255.0, axis=0)
#     # reverse channel
...
def preprocess_image(image_path, input_size):
    ...
    resized_image = np.array(original_image.resize((224,224), resample=Image.Resampling.BILINEAR))
...
def nms(results: List[AIResult]) -> List[AIResult]:
    ...
    while pq:
        ...
        while pq:
            ...
            if box_iou(max_result.rect, detection.rect) >= 0.25 and detection.detection_class == max_result.detection_class:  # IoU threshold
                continue
...
def nms2(results: List[AIResult]) -> List[AIResult]:
    ...
    while priority_adjusted_results:
        ...
        remaining_results = []
        while priority_adjusted_results:
            score, detection = heapq.heappop(priority_adjusted_results)
            if box_iou(max_result.rect, detection.rect) < 0.3:
                remaining_results.append((score, detection))
...
def postprocess(outputs, threshold, num_classes, input_size):
    ...
    filtered_results2 = nms2(filtered_results)
    return filtered_results2
...
def get_food_area_inside_tray_after(tray: AIResult, results: List[AIResult]) -> float:
    """
...
def get_food_area_inside_tray_before(tray: AIResult, results: List[AIResult]) -> float:
    """
    Calculate the total area of food inside the tray, excluding overlapping areas.
    Special handling for class 5 objects in bottom right region.
    """
...
def run_entire_pipeline(model_path, webp):
    ...
    if tray is None:
        return -1, label_exists, -1
    return detection_results, tray, label_exists

def run_entire_pipeline_after(model_path, webp):
    detection_results, tray, label_exists = run_entire_pipeline(model_path, webp)
    if detection_results == -1:
        return -1, -1, -1, -1, -1
    food_in_tray = get_food_area_inside_tray_after(tray, detection_results)
...
def run_entire_pipeline_before(model_path, webp):
    detection_results, tray, label_exists = run_entire_pipeline(model_path, webp)
    if detection_results == -1:
        return -1, -1, -1, -1, -1
    food_in_tray = get_food_area_inside_tray_before(tray, detection_results)
    tray_area = (tray.rect.right - tray.rect.left) * (tray.rect.bottom - tray.rect.top)
    zero_waste_score = food_in_tray / tray_area
    return_zero_waste_score = float(zero_waste_score)
    if zero_waste_score >= 0.3:
